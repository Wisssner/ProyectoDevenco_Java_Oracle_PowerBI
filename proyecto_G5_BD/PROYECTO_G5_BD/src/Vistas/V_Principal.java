...
import Controlador.ExternoC_Partida;
import Controlador.C_Proyecto;
import Controlador.ExternoC_Proy_Partida; 
import Controlador.C_Proveedor;
import Controlador.C_Partida_Mezcla;
import Controlador.ExternoC_Partida_Mezcla;
import Controlador.C_Proy_Partida;
import Controlador.C_Proy_Partida_Mezcla;/*Controlador Interno*/
import Controlador.ExternoC_Proy_Partida_Mezcla;/*Controlador Externo*/
import Controlador.C_Register;
import Controlador.C_VtaCompPagoCab;
import Controlador.ExternoC_DProy_Partida_Mezcla;
...
public class V_Principal extends javax.swing.JFrame {
    ...
    public void init(){
        ...
        menu.addEvent(new EventMenuSelected() {
            ...
            public void menuSelected(int index,int subindex) {
                ...
                }else if (index == 3) { // NUEVO MENU P.PARTIDAS
                    if (subindex == 0) {
                        V_ExternoPartida vnv = new V_ExternoPartida();
                        mainBody.displayForm(vnv, "Partidas Externas");
                        // Si tienes un controlador, lo instancias aquí.
                        ExternoC_Partida cnv = new ExternoC_Partida(vnv);
                    }else if(subindex==1){
                        V_ExternoPartida_Mezcla vpm=new V_ExternoPartida_Mezcla();
                        mainBody.displayForm(vpm,"Partida Mezcla Externas");
                        ExternoC_Partida_Mezcla cp=new ExternoC_Partida_Mezcla(vpm);
                    }
                }else if (index == 4) {//PROYECTOS
                    if(subindex==0){
                        ...
                        mainBody.displayForm(vppm,"Partidas Mezcla interna");
                        C_Proy_Partida_Mezcla cppc=new C_Proy_Partida_Mezcla(vppm);
                    }
                }else if (index == 5) {// P.PROYECTOS  ----- CAMBIAR VISTA, CONTROLADOR
                    if(subindex==0){
                        V_Proyecto vp=new V_Proyecto();
                        mainBody.displayForm( vp,"Proyecto");
                        C_Proyecto cp=new C_Proyecto(vp);
                    }else if(subindex==1){
                        //V_ExterboPartida_Proyecto vpp=new V_ExternoPartida_Proyecto();
                        V_ExternoProy_Partida vppp = new V_ExternoProy_Partida();
                        mainBody.displayForm(vppp,"Partidas");
                        ExternoC_Proy_Partida cppp = new ExternoC_Proy_Partida(vppp);
                        //ExternoC_Partida_Proyecto cpp=new ExternoC_Partida_Proyecto(vpp);
                    }else if(subindex==2){
                        V_ExternoProy_Partida_Mezcla vppma =new V_ExternoProy_Partida_Mezcla();
                        mainBody.displayForm(vppma,"Partidas");
                        ExternoC_Proy_Partida_Mezcla cppca=new ExternoC_Proy_Partida_Mezcla(vppma);
                    }
                }else if (index == 6) {//PRESUPUESTO
                    if(subindex==0){
                ...
                }else if (index == 7) {// P.PRESUPUESTO  ---- CAMBIAR VISTA,CONTROLADOR.
                    if(subindex==0){
                        V_ExternoDProy_Partida_Mezcla vdppm =new V_ExternoDProy_Partida_Mezcla();
                        mainBody.displayForm(vdppm,"Partidas");
                        ExternoC_DProy_Partida_Mezcla cdppm=new ExternoC_DProy_Partida_Mezcla(vdppm);
                    }
                }else if (index == 8) {//VENTAS
                    if(subindex==0){
                        V_Venta vv=new V_Venta();
                        mainBody.displayForm( vv,"VtaCompPagoCab");
                        C_VtaCompPagoCab cc=new C_VtaCompPagoCab(vv);
                    }
                }else if (index == 9) {//EGRESOS
                    if(subindex==0){
                        V_Compra vc=new V_Compra();
                        mainBody.displayForm( vc,"CompPagoCab");
                        C_CompPagoCab cc=new C_CompPagoCab(vc);
                    }
                }else if (index == 10) {//FLUJO DE CAJA
                    if(subindex==0){
                        V_FlujoCajaEsp vfce=new V_FlujoCajaEsp();
                        vfce.setVisible(true);
                    }
                }else if (index == 11) {//P.FLUJO DE CAJA --- CAMBIAR VISTA, CONTROLADOR
                    if(subindex==0){
                        V_ExternoFlujoCajaEsp vfce=new V_ExternoFlujoCajaEsp();
                        vfce.setVisible(true);
                    }
                }else if (index == 12) {//ADMINISTRADOR
                    if(subindex==0){
                        V_Compania vc=new V_Compania();
                        mainBody.displayForm( vc,"Compania");
                        C_Compania cc=new C_Compania(vc);
                    }
                }else if (index == 13) {//CONFIGURACION
                    if(subindex==0){
    ...
    private void initComponents() {
        ...
        menu.setBackground(new java.awt.Color(0, 0, 18));
