...
public class Proy_Partida_MezclaDAO implements CRUD <Proy_Partida_Mezcla>{
    ...
    public List<Proy_Partida_Mezcla> listar() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Set values from ResultSet to Proy_Partida_Mezcla object
                lista.add(ppm);
            }
    ...
    public int add(Proy_Partida_Mezcla p) {
        String sql = "{call INSERTAR_PROY_PARTIDA_MEZCLA(?,?,?,?,?,?)}";
        try {
            con = conexion.conectar();
            myCall = con.prepareCall(sql);
    ...
    public int actualizar(Proy_Partida_Mezcla p) {
        String sql = "UPDATE PROY_PARTIDA_MEZCLA SET NROVERSION=?, NIVEL=?, ORDEN=?, COSTOUNIT=?, CANT=?, COSTOTOT=? WHERE CODCIA=? AND CODPYTO=? AND INGEGR=? AND CORR=? AND CONDPARTIDA=?";
        try {
            con = conexion.conectar();
            pst = con.prepareStatement(sql);
    ...
    public void eliminar(int id) {
        String sql = "DELETE FROM PROY_PARTIDA_MEZCLA WHERE CORR=?";
        try {
            con = conexion.conectar();
            pst = con.prepareStatement(sql);
            pst.setInt(1, id);
    ...
    private void closeResources() {
        try {
            ...
            JOptionPane.showMessageDialog(null, "Excepcion al cerrar recursos.\n" + e.toString());
        }
    }
