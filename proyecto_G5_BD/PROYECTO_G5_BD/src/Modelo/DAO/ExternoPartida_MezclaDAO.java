...
public class ExternoPartida_MezclaDAO implements CRUD<ExternoPartida_Mezcla>{
    ...
    @Override
    public List listar() {
        List<ExternoPartida_Mezcla> lista = new ArrayList<>();
    ...
    public int add(ExternoPartida_Mezcla p) {
        System.out.println("{call INSERTAR_PPARTIDA_MEZCLA(?,?,?,?,?,?,?,?,?,?)}");
        try {
            con = conexion.conectar();

            // Verificar existencia de las claves foráneas
            String checkSql = "SELECT 1 FROM ELEMENTOS WHERE CodTab = ? AND CodElem = ?";
            pst = con.prepareStatement(checkSql);
            pst.setString(1, p.gettUnitMed());
            pst.setString(2, p.geteUnitMed());
            rs = pst.executeQuery();

            if (!rs.next()) {
                throw new SQLException("Clave foránea no encontrada en la tabla ELEMENTOS.");
            }
            rs.close();
            pst.close();
    ...
    @Override
    public int actualizar(ExternoPartida_Mezcla p) {
    ...
    public void eliminarDatos(int cia,int id,String tipo,int cor) {
       String sql1="DELETE from PPARTIDA_MEZCLA where CODCIA="+cia+" AND CODPARTIDA="+id+" AND INGEGR='"+tipo+"' AND CORR="+cor;
        try{
            con=conexion.conectar();
    ...
    public ExternoPartida_Mezcla listarId(int cia,String tipo,int par,int id){
        ExternoPartida_Mezcla pm = new ExternoPartida_Mezcla();
        String sql="SELECT CORR,CODPARTIDA,PADCODPARTIDA,NIVEL,ORDEN,TUNIMED,EUNIMED,"
                + "COSTOUNIT,VIGENTE FROM PPARTIDA_MEZCLA WHERE CODCIA="+cia+" AND INGEGR='"+tipo+"' AND CODPARTIDA="+par+" AND CORR="+id;
        try{
            con = conexion.conectar();
    ...
    public ExternoPartida_Mezcla listarOrden(int cia,String tipo,int par,int nivel,int orden){
        ExternoPartida_Mezcla pm = new ExternoPartida_Mezcla();
        String sql;
        if(par==0){
    ...
    public List listarPorCodCia(int id,String tip){
        ...
        String sql="SELECT "
                ...
                + "FROM PPARTIDA_MEZCLA m WHERE m.codcia="+id+" AND m.ingegr='"+tip+"' order by m.corr";
        System.out.println(sql);
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
    ...
    public List listarCodPartida_Mezcla(int id,String tip){
        ...
        String sql="SELECT "
                + "codpartida"
                + " FROM PPARTIDA_MEZCLA WHERE codcia="+id+" AND ingegr='"+tip+"' order by codpartida";
        System.out.println(sql);
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
    ...
    public List listarMezclas(int id,String tip){
        List<ExternoPartida_Mezcla> lista = new ArrayList<>();
        String sql="SELECT DISTINCT PADCODPARTIDA FROM PPARTIDA_MEZCLA "
    ...
    public List listarMezclasPadre(int id,String tip){
        List<ExternoPartida_Mezcla> lista = new ArrayList<>();
        String sql="SELECT DISTINCT CODPARTIDA FROM PPARTIDA_MEZCLA "
    ...
    public int asignarOrden(String tip,int nivel,int cia){
        int orden=0;
        String sql="SELECT * FROM (SELECT ORDEN FROM PPARTIDA_MEZCLA WHERE INGEGR='"+tip+"' AND NIVEL="+nivel+" AND CODCIA="+cia+" ORDER BY ORDEN DESC) WHERE ROWNUM=1";
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery(sql);
    ...
    public int asignarOrdenNoNULL(String tip,int cod,int cia){
        int orden=0;
        String sql="SELECT * FROM (SELECT ORDEN FROM PPARTIDA_MEZCLA WHERE INGEGR='"+tip+"' AND PADCODPARTIDA="+cod+" AND CODCIA="+cia+" ORDER BY ORDEN DESC) WHERE ROWNUM=1";
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery(sql);
    ...
    public int asignarNivelNoNULL(String tip,int cod,int cia){
        int nivel=0;
        String sql="SELECT * FROM (SELECT NIVEL FROM PPARTIDA_MEZCLA WHERE INGEGR='"+tip+"' AND CODPARTIDA="+cod+" AND CODCIA="+cia+" ORDER BY ORDEN DESC) WHERE ROWNUM=1";
        try{
            con=conexion.conectar();
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery(sql);
