...
public class C_Partida implements ItemListener,ActionListener, KeyListener, MouseListener{
    ...
    /*SE AGREGO ESTE METODO PARA DESCARGAR EL PDF*/
    public void descargarDatos() {
        try {
            // Nombre del archivo PDF que se generará
            String nombreArchivo = "reporte_partidas.pdf";
            ...
            Table tableI = new Table(vp.tablaPartida_I.getColumnCount());
            for (int i = 0; i < vp.tablaPartida_I.getColumnCount(); i++) {
                tableI.addHeaderCell(new Cell().add(new Paragraph(vp.tablaPartida_I.getColumnName(i))));
            }
            for (int row = 0; row < vp.tablaPartida_I.getRowCount(); row++) {
                for (int col = 0; col < vp.tablaPartida_I.getColumnCount(); col++) {
                    tableI.addCell(new Cell().add(new Paragraph(vp.tablaPartida_I.getValueAt(row, col).toString())));
                }
            }
            document.add(tableI);
            ...
            Table tableE = new Table(vp.tablaPartida_E.getColumnCount());
            for (int i = 0; i < vp.tablaPartida_E.getColumnCount(); i++) {
                tableE.addHeaderCell(new Cell().add(new Paragraph(vp.tablaPartida_E.getColumnName(i))));
            }
            for (int row = 0; row < vp.tablaPartida_E.getRowCount(); row++) {
                for (int col = 0; col < vp.tablaPartida_E.getColumnCount(); col++) {
                    tableE.addCell(new Cell().add(new Paragraph(vp.tablaPartida_E.getValueAt(row, col).toString())));
                }
            }
            document.add(tableE);
