...
public class C_DProy_Partida_Mezcla implements ActionListener,MouseListener,ItemListener{
    ...
    public void descargarDatos() {
        try {
            ...
            String nombreArchivo = "reporte_PresupuestoPartidas.pdf";
            ...
            document.add(new Paragraph("Reporte de Partidas"));

            // Agregar tabla para la parte I
            document.add(new Paragraph("Tabla Partida I"));
            Table tableI = new Table(vpm.tablaDProy_Partida_I.getColumnCount());
            for (int i = 0; i < vpm.tablaDProy_Partida_I.getColumnCount(); i++) {
                tableI.addHeaderCell(new Cell().add(new Paragraph(vpm.tablaDProy_Partida_I.getColumnName(i))));
            }
            for (int row = 0; row < vpm.tablaDProy_Partida_I.getRowCount(); row++) {
                for (int col = 0; col < vpm.tablaDProy_Partida_I.getColumnCount(); col++) {
                    tableI.addCell(new Cell().add(new Paragraph(vpm.tablaDProy_Partida_I.getValueAt(row, col).toString())));
                }
            }
            document.add(tableI);
            ...
            Table tableE = new Table(vpm.tablaDProy_Partida_E.getColumnCount());
            for (int i = 0; i < vpm.tablaDProy_Partida_E.getColumnCount(); i++) {
                tableE.addHeaderCell(new Cell().add(new Paragraph(vpm.tablaDProy_Partida_E.getColumnName(i))));
            }
            for (int row = 0; row < vpm.tablaDProy_Partida_E.getRowCount(); row++) {
                for (int col = 0; col < vpm.tablaDProy_Partida_E.getColumnCount(); col++) {
                    tableE.addCell(new Cell().add(new Paragraph(vpm.tablaDProy_Partida_E.getValueAt(row, col).toString())));
                }
            }
            document.add(tableE);
