...
public class C_Proy_Partida implements ActionListener, KeyListener, MouseListener{
    ...
    public void descargarDatos() {
        try {
            ...
            String nombreArchivo = "reporte_proyectos_partidas.pdf";
            ...
            document.add(new Paragraph("Reporte de Proyectos y Partidas"));

            // Agregar tabla para Proy_Partida_I
            document.add(new Paragraph("Tabla de Proyectos y Partidas I"));
            Table tableI = new Table(modelProy_PartidaI.getColumnCount());
            addTableHeaders(tableI, modelProy_PartidaI);
            addTableData(tableI, modelProy_PartidaI);
            document.add(tableI);

            // Agregar tabla para Proy_Partida_E
            document.add(new Paragraph("Tabla de Proyectos y Partidas E"));
            Table tableE = new Table(modelProy_PartidaE.getColumnCount());
            addTableHeaders(tableE, modelProy_PartidaE);
            addTableData(tableE, modelProy_PartidaE);
            document.add(tableE);
    ...
    private void addTableHeaders(Table table, DefaultTableModel model) {
        for (int i = 0; i < model.getColumnCount(); i++) {
            table.addHeaderCell(new Cell().add(new Paragraph(model.getColumnName(i))));
        }
    }

    // Método para agregar datos a la tabla
    private void addTableData(Table table, DefaultTableModel model) {
        for (int row = 0; row < model.getRowCount(); row++) {
            for (int col = 0; col < model.getColumnCount(); col++) {
                table.addCell(new Cell().add(new Paragraph(model.getValueAt(row, col).toString())));
            }
        }
    }
